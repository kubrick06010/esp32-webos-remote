...
</style></head><body><main class="app"><header class="top"><b>▣ ESP32 webOS Remote</b><span>ESP32 · televisor.local</span><span id="s"><i class="dot"></i> Connecting…</span></header><section class="remote">
...
bool dispatchCommand(const String &command, String &message) {
  ...
  if (command == "POWER" && !reachable) {
    ...
    sendWakeOnLan(); message = "Wake-on-LAN packet sent"; return true;
  }
  if (!reachable) {
    controlWs.disconnect(); pointerWs.disconnect();
    controlConnected = registered = pointerConnected = false;
